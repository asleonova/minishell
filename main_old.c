...
int ft_print_env(t_data *data, t_commands *command)
{
    int i;
...
int main() // testing unset env func
{
	...
	command->count_args = 0;
	data->envp = (char**)malloc(sizeof(char *) * 4 + 1);
	data->envp[0] = ft_strdup("ATH=dakljdaldaj");
	data->envp[1] = ft_strdup("ZSHC=/Users/dbliss/.oh-my-zsh");
	data->envp[2] = ft_strdup("CSERR=dbliss");
	data->envp[3] = ft_strdup("sfkjsfklfsjl=flsfl=");
	data->envp[4] = NULL;
	command->lst = ft_lstnew("anna=");
	append_lst(&command->lst, "ZSHC=test");
